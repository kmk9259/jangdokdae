...
class PipelineStore:
    ...
    async def save_pipeline_run(
        ...
        """파이프라인 전 단계 결과를 단일 트랜잭션으로 저장."""
        async with AsyncSessionLocal() as session:
            async with session.begin():
                article_id_map = await self._save_articles(session, articles)
                cluster_id_map = await self._save_clusters(session, clusters, run_date, article_id_map)
                await self._save_entity_extraction(session, clusters, cluster_id_map)
                await self._save_company_data(session, clusters)
        logger.info("[store] pipeline run saved run_date=%s articles=%d", run_date, len(articles))

    # ── 내부 세션 인자 메서드 ─────────────────────────────────────────────────

    async def _save_articles(self, session: Any, articles: list[dict]) -> dict[str, int]:
        if not articles:
            return {}
    ...
    async def _save_clusters(
        ...
        cluster_id_map: dict[int, int] = {row.cluster_seq: row.id for row in result}
    ...
    async def _save_entity_extraction(
        self,
        session: Any,
        clusters: list[dict],
    ...
    async def _save_company_data(self, session: Any, clusters: list[dict]) -> None:
        seen: set[str] = set()
